...
const SquishyCard = ({
  ...
  const BGs = [BackgroundA, BackgroundB, BackgroundC];

  const Background = BGs["ABC".indexOf(bg)]!;
  return (
    <motion.div
      whileHover="hover"
      transition={{
      ...
      className="relative h-72 w-full shrink-0 overflow-hidden rounded-xl bg-primary-light p-8 lg:w-[32rem] dark:bg-primary-dark"
    >
      <div className="relative z-10 text-white">
        <div className="mb-3 flex w-fit items-center gap-2 rounded-full bg-white/30 px-3 py-0.5 text-sm font-light text-white">
          <Icon />
          <span>{title}</span>
        </div>
        <motion.span
